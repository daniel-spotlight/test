...
SET
	...
	dc.semsInProgressGS  = CASE WHEN (gp.semsComplete + gp.semsInProgress + (dc.semsComplete - dc.primaryComplete)) >= gp.semsReqdTotal
							 ...
							 END
WHERE gp.semsReqdTotal > (gp.semsComplete + gp.semsInProgress) and upper(dc.primarySubj) in ('BIOLOGICAL SCIENCE', 'PHYSICAL SCIENCE') and dc.studentID in (select studentID from grad_progress where subject = 'General Science');
...
SET
	...
	dc.secondaryInProgress = CASE WHEN (gp.semsComplete + gp.semsInProgress + (dc.semsComplete - dc.primaryComplete - dc.semsCompleteGS)) >= gp.semsReqdTotal
								  THEN 0
                                  WHEN (gp.semsComplete + gp.semsInProgress + (dc.semsComplete - dc.primaryComplete - dc.semsCompleteGS) + (dc.semsInProgress - dc.primaryInProgress - dc.semsInProgressGS)) >= gp.semsReqdTotal
                                  THEN gp.semsReqdTotal - (gp.semsComplete + gp.semsInProgress) -- (dc.semsInProgress - dc.primaryInProgress) - (gp.semsReqdTotal - (gp.semsComplete + gp.semsInProgress + (dc.semsComplete - dc.primaryComplete)))
...
select dc.id,
  ...
  secondarySubj
    ...
    where UPPER(`secondarySubj`) in ('Biological Science','Physical Science') and dc.studentID in (select studentID from grad_progress where subject = 'General Science');
...
update dual_courses dc  #add the calculated inProgress overflow to semsInProgressGS 
    ...
    and dc.id = gsio.id 
    set dc.semsInProgressGS = dc.semsInProgressGS+gsio.inpOverflow;

update dual_courses dc  #add the calculated complete overflow to semsCompleteGS
	inner join gen_sci_sems2 gsc
    on dc.id=gsc.id
	set dc.semsCompleteGS = dc.semsCompleteGS+gsc.subjectOverflow;
...
SET
    ...
    gp.semsInProgress = gp.semsInProgress + dcs.primaryInProgressSum;
...
SET
    ...
    gp.semsInProgress = gp.semsInProgress + dcs.secondaryInProgressSum; #originally 317
...
SET
    ...
    gp.semsInProgress = gp.semsInProgress + dcs.inProgressGSSum
